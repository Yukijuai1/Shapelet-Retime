...
class PNTripletLoss(torch.nn.modules.loss._Loss):
    ...
    def __init__(self, compared_length, cuda, gpu):
    ...
    def forward(self, batch, encoder, params, save_memory=False):
     ...
     for m in range(slide_num):
         ...
         # !!! 报错：can't convert cuda:0 device type tensor to numpy.
         ...
         for i in range(num_cluster):
             ...
             if self.cuda:
              dist_positive = dist_positive.cuda(self.gpu)
              dist_intra_positive = dist_intra_positive.cuda(self.gpu)
              dist_intra_negative = dist_intra_negative.cuda(self.gpu)
              dist_negative = dist_negative.cuda(self.gpu)
              loss_cluster = loss_cluster.cuda(self.gpu)
             ...
             # !!! 这里是该完以后的
             if self.cuda:
                representation_anc = representation_anc.cuda(self.gpu)
             # !!! 检查设备
             #  print("====================================================")
             #  print(representation_anc.device)
             #  print("====================================================")
             ...
             for l in range(1,num_positive+1):
                 ...
                 if self.cuda:
                    representation_pos = representation_pos.cuda(self.gpu)
                 # encode positive
                 ...
                 if self.cuda:
                    anchor_minus_positive = anchor_minus_positive.cuda(self.gpu)
             ...
             if num_positive > 1:
                 ...
                 if self.cuda:
                   representation_intra_pos_0 = representation_intra_pos_0.cuda(self.gpu)
                 ...
                 if self.cuda:
                   representation_intra_pos_1 = representation_intra_pos_1.cuda(self.gpu)
             ...
             for k in range(num_cluster):
                 ...
                 if self.cuda:
                   dist_cluster_k_negative = dist_cluster_k_negative.cuda(self.gpu)
                 if k == i:
                     continue
                 else:
                     # select negative
                     ...
                     for j in range(num_negative_cluster_k):
                         ...
                         if self.cuda:
                           representation_neg = representation_neg.cuda(self.gpu)
                 ...
                 if num_negative_cluster_k > 1:
                     ...
                     if self.cuda:
                       representation_intra_neg_0 = representation_intra_neg_0.cuda(self.gpu)
                     ...
                     if self.cuda:
                       representation_intra_neg_1 = representation_intra_neg_1.cuda(self.gpu)
